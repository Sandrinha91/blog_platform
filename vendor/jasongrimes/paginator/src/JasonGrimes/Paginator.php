...
class Paginator
{
    ...
    // my added functions
    public function getStartFromArray()
    {
        return $this->arrayStartFrom;
    }

    protected function addStartFrom($key, $val)
    {
        $this->arrayStartFrom[$key] = $val;
    }

    public function updateArrayStart($from, $to , $increaseFor)
    {
        $startFrom = 0;

        for (  $i = $from; $i <= $to; $i++ )
        {
            $this->addStartFrom( $startFrom, $i);
            $startFrom += $increaseFor;
        }
    }

    public function getStartValueByPage($page)
    {
        $array = $this->getStartFromArray();

        $start = array_search($page,$array);
        return $start;
    }
    // my finished adding functions
